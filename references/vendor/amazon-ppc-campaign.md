# 【已采纳｜内部参考】Amazon PPC Campaign Optimization（上游原文）

> **本文件是 vendored 的上游方法论，不是独立技能。** 唯一入口仍是本 Skill 的 SKILL.md。
>
> - 上游：`nexscope-ai/Amazon-Skills` → `amazon-ppc-campaign`
> - 作者/版权：Nexscope AI 及其贡献者；上游仓库自带 LICENSE
> - 采纳日期：2026-09-21
> - **安全审查结论**：正文为纯 Markdown 方法论，无脚本执行、无外联、无凭据读取。**已剔除**上游附带的 `scripts/fetch-competitor.sh`（理由见文末「取舍说明」）。
> - 引用规则：读它以补方法论细节；**凡与本 Skill 的 `metrics.md` / `diagnosis.md` 冲突处，一律以本 Skill 为准**（差异见文末）。
...
**已吸收进本 Skill 的要点**
1. **双目标 ACoS**：成熟期目标 + 新品期目标（更宽松），已并入 `playbooks.md` 场景 A。
2. **初始出价的相对关系**：Exact=基准，Broad=Exact 的 70%~80%，Auto 与商品定位=50%~70% —— 已并入 `playbooks.md`。
3. **分档降价规则**（比本 Skill 原规则更细）：>200% 砍 30%~50%；100%~199% 砍 20%；目标+10%~99% 砍 10%~15%；±10% 不动；低于目标提 10%~20% —— 已并入 `diagnosis.md`。
4. **「亚马逊建议出价 vs Max CPC」缺口决策**：建议出价高于可承受 CPC 时，三选一（接受亏损冲排名 / 提价 / 跳过该词）—— 已并入 `actions.md`。
5. **5 维度审计**（含「错失机会」维度）：已在 `diagnosis.md` 第 2 层与 `reporting.md` 补入。
6. **通用浪费修饰词表**：free / cheap / used / DIY / review / reddit / how to —— 已并入 `actions.md`。
7. **亚马逊自动补全接口**：已实现为 `scripts/ads_opt.mjs keywords` 子命令（免登录、官方端点）。

**未采纳：`scripts/fetch-competitor.sh`**
- 它用浏览器 UA `curl` 抓 `amazon.<域>/dp/<ASIN>` 商品页，再 grep 解析 HTML。
- 两条理由剔除：① 与本 Skill 的合规红线冲突（只用官方 API 与后台报表取数，不抓页面）；② 依赖 `productTitle"` 等 HTML 片段，亚马逊风控与改版下成功率低，不可靠。
- 替代路径：商品标题/五点/BSR 走官方商品数据能力或已装的跨境调研技能；关键词发现走 `ads_opt.mjs keywords`。

**口径冲突（以本 Skill 为准）**
- 上游示例的 break-even ACoS **未扣退货损耗**，且把 Amazon fees 合并成一个笼统数字；本 Skill 要求逐项扣（FBA / 佣金 / 头程 / 退货率），口径更严，结论更保守。
- 上游用「行业平均转化率 10%~15%」兜底；本 Skill 要求优先用本品实际 CVR，缺失时明确标注为假设。
